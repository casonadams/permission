...
describe("mergeFlatPermissions", () => {
  ...
  test("object replaces string", () => {
    const result = mergeFlatPermissions({ tools: "ask" }, { tools: { Write: "deny" } });
    expect(result).toEqual({ tools: { Write: "deny" } });
  });

  test("string replaces object", () => {
    const result = mergeFlatPermissions({ tools: { Write: "deny" } }, { tools: "allow" });
    expect(result).toEqual({ tools: "allow" });
  });
  ...
  test("preserves keys only in base", () => {
    const result = mergeFlatPermissions({ tools: "ask", bash: "deny" }, { tools: "allow" });

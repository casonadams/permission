...
function makeResolver(pm?: ScopedPermissionManager, sessionRules?: Pick<SessionRules, "getRuleset">) {
...
describe("PermissionResolver", () => {
  describe("resolve", () => {
    it("forwards surface, input, and agentName, applying the empty session ruleset", () => {
      ...
      expect(permissionManager.checkPermission).toHaveBeenCalledWith("bash", { command: "ls" }, "agent-x", []);
    });
    ...
    it("defaults agentName to undefined when omitted", () => {
      ...
      expect(permissionManager.checkPermission).toHaveBeenCalledWith("read", { path: ".env" }, undefined, []);
    });
  ...
  describe("resolvePathPolicy", () => {
    it("forwards values and agentName with the current session ruleset", () => {
      ...
      expect(permissionManager.checkPathPolicy).toHaveBeenCalledWith(["/proj/src/a.ts", "src/a.ts"], "agent-x", []);
    });
    ...
    it("applies a recorded session approval on the next call", () => {
      ...
      sessionRules.recordSessionApproval(SessionApproval.single("path", "src/*"));
  ...
  describe("checkPermission", () => {
    it("delegates to permissionManager.checkPermission with the given args", () => {
      ...
      expect(permissionManager.checkPermission).toHaveBeenCalledWith("bash", { command: "ls" }, "agent-1", undefined);
    });

    it("passes optional sessionRules through when supplied", () => {
      const { resolver, permissionManager } = makeResolver();
      const extraRules: Ruleset = [{ surface: "bash", pattern: "*", action: "allow", origin: "session" }];

      resolver.checkPermission("bash", { command: "ls" }, undefined, extraRules);

      expect(permissionManager.checkPermission).toHaveBeenCalledWith("bash", { command: "ls" }, undefined, extraRules);
    });
  });

...
describe("requestPermissionDecisionFromUi", () => {
  ...
  it("passes four options to ui.select", async () => {
    ...
    expect(options).toEqual(["Yes", "Yes, for this session", "No", "No, provide reason"]);
  });
  ...
  it("still returns approved_for_session when user selects the custom session label", async () => {
    ...
    const result = await requestPermissionDecisionFromUi(ui, "Title", "Message", { sessionLabel: customLabel });
    expect(result).toEqual({ approved: true, state: "approved_for_session" });
  });
...
describe("approved permission decision helpers", () => {
  it("creates an approved decision", () => {
    expect(createApprovedPermissionDecision()).toEqual({ approved: true, state: "approved" });
  });

...
import { buildDirectUiPrompt, buildForwardedUiPrompt, buildRpcUiPrompt } from "#src/prompting/permission-ui-prompt";
...
describe("buildDirectUiPrompt", () => {
  ...
  it("prefers explicit prompt surface and value", () => {
    expect(
      buildDirectUiPrompt({
        requestId: "req-4",
        source: "tool_call",
...
describe("buildRpcUiPrompt", () => {
  ...
  it("defaults missing surface, value, and agentName to null", () => {
    expect(buildRpcUiPrompt({ requestId: "req-rpc-2", message: "Allow?" })).toEqual({
      requestId: "req-rpc-2",
      source: "rpc_prompt",
      surface: null,
      value: null,

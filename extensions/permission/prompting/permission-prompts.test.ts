...
function formatAskPrompt(...args: FormatAskPromptTuple): string {
  const [result, agentName, input, formatter] = args;
  return formatAskPromptFromArgs({ result, agentName, input, formatter });
}

function makeFormatter(lookup?: ToolInputFormatterLookup): ToolPreviewFormatter {
...
function toolResult(toolName: string, overrides: Partial<PermissionCheckResult> = {}): PermissionCheckResult {
  return {
    toolName,
    state: "ask",
    source: "tool",
...
function mcpResult(target: string, overrides: Partial<PermissionCheckResult> = {}): PermissionCheckResult {
  return {
    toolName: "mcp",
    target,
    state: "ask",
...
describe("formatAskPrompt", () => {
  test("uses 'Current agent' when no agent name given", () => {
    const result = formatAskPrompt(toolResult("read"), undefined, { path: "/src" }, makeFormatter());
    expect(result).toContain("Current agent");
  });

  test("uses agent name when provided", () => {
    const result = formatAskPrompt(toolResult("read"), "my-agent", { path: "/src" }, makeFormatter());
  ...
  test("suppresses full-command suffix when input command matches the sub-command (no chain)", () => {
    ...
    expect(result).toBe("Current agent requested bash command 'git push'. Allow this command?");
  });

  test("suppresses full-command suffix when input is undefined", () => {
    const result = formatAskPrompt(toolResult("bash", { command: "git push" }), undefined, undefined, makeFormatter());
    expect(result).not.toContain("full command:");
  });
  ...
  test("formats bash prompt with nested execution context", () => {
    ...
    expect(result).toContain("bash command 'rm -rf foo' (matched 'rm *', inside command substitution).");
  });

  test("formats MCP prompt with target", () => {
    const result = formatAskPrompt(mcpResult("server:query"), undefined, undefined, makeFormatter());
  ...
  test("includes real input preview for non-bash non-mcp tools", () => {
    const result = formatAskPrompt(toolResult("read"), undefined, { path: "/src/foo.ts" }, makeFormatter());
    expect(result).toContain("path '/src/foo.ts'");
    expect(result).toContain("Allow this call?");
  });

  test("omits input suffix when formatter returns empty string for input", () => {
    const result = formatAskPrompt(toolResult("task"), undefined, {}, makeFormatter());
...
describe("formatSkillPathAskPrompt", () => {
  test("includes skill name, read path, and agent name", () => {
    const result = formatSkillPathAskPrompt(skillEntry("librarian"), "/skills/librarian/SKILL.md", "my-agent");
    expect(result).toContain("librarian");
  ...
  test("uses 'Current agent' without agent name", () => {
    const result = formatSkillPathAskPrompt(skillEntry("librarian"), "/skills/librarian/SKILL.md");
    expect(result).toContain("Current agent");
  });
});

...
export function formatPathDenyBody(ctx: Extract<DenialContext, { kind: "path" }>): string {
  if (ctx.matchedPattern)
    return `Path denied: '${abbreviateHomePath(ctx.pathValue)}' (matched '${ctx.matchedPattern}').${reasonSuffix(ctx.reason)}`;
  if (ctx.cwd) return `External path denied: '${abbreviateHomePath(ctx.pathValue)}'.`;

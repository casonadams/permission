...
import { resolvePermissionForwardingTargetSessionId } from "#src/forwarding/permission-forwarding";
import { canResolveAskPermissionRequest, shouldAutoApprovePermissionState } from "#src/prompting/yolo-mode";
...
describe("shouldAutoApprovePermissionState", () => {
  test("returns true for 'ask' when yolo mode is on", () => {
    expect(shouldAutoApprovePermissionState("ask", makeConfig(true))).toBe(true);
  });

  test("returns false for 'ask' when yolo mode is off", () => {
    expect(shouldAutoApprovePermissionState("ask", makeConfig(false))).toBe(false);
  });

  test("returns false for 'ask' when yolo mode is undefined", () => {
    expect(shouldAutoApprovePermissionState("ask", makeConfig(undefined))).toBe(false);
  });

  test("returns false for 'allow' even when yolo mode is on", () => {
    expect(shouldAutoApprovePermissionState("allow", makeConfig(true))).toBe(false);
  });

  test("returns false for 'deny' even when yolo mode is on", () => {
    expect(shouldAutoApprovePermissionState("deny", makeConfig(true))).toBe(false);
  });
});
...
test("Yolo mode only auto-approves ask-state permissions", () => {
  expect(shouldAutoApprovePermissionState("ask", DEFAULT_EXTENSION_CONFIG)).toBe(false);
  expect(

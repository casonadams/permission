...
import { buildInputForSurface, normalizeInput } from "#src/policy/input-normalizer";
import { createMcpPermissionTargets } from "#src/policy/mcp/mcp-targets";
...
describe("buildInputForSurface", () => {
  it("preserves path values for public path checks", () => {
    expect(buildInputForSurface("path", ".env")).toEqual({ path: ".env" });
  });

  it("preserves path values for public path-bearing tool checks", () => {
    for (const surface of ["read", "write", "edit", "find", "grep", "ls"]) {
      expect(buildInputForSurface(surface, "/tmp/file")).toEqual({ path: "/tmp/file" });
    }
  });
  ...
  it("uses status input for missing MCP values", () => {
    expect(buildInputForSurface("mcp", undefined)).toEqual({});
  });
});

...
import { LocalPermissionsService, type LocalPermissionsServiceDeps } from "#src/permissions-service";
...
function makeService(overrides: ServiceOverrides = {}) {
  const deps = makeServiceDeps(overrides);
  const service = new LocalPermissionsService(deps);
  return { service, ...deps };
}

function makeServiceDeps(overrides: ServiceOverrides): LocalPermissionsServiceDeps {
  return {
    permissionManager: overrides.permissionManager ?? makeFakePermissionManager(),
    sessionRules: overrides.sessionRules ?? makeSessionRules(),
    formatterRegistry: overrides.formatterRegistry ?? makeFormatterRegistry(),
    accessExtractorRegistry: overrides.accessExtractorRegistry ?? makeAccessExtractorRegistry(),
  };
}

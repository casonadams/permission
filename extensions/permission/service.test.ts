...
describe("service adapter delegation", () => {
  ...
  it("checkPermission passes path values for path-bearing tool surfaces", () => {
    const checkPermission = vi.fn().mockReturnValue(fakeResult);
    ...
    expect(checkPermission).toHaveBeenCalledWith("read", { path: "/tmp/file" }, undefined, []);
  });
  ...
  it("checkPermission uses empty object for unknown surfaces", () => {
    ...
    publishPermissionsService(service);
    getPermissionsService()!.checkPermission("unknown", "/tmp/file");

    expect(checkPermission).toHaveBeenCalledWith("unknown", {}, undefined, []);
  });
});

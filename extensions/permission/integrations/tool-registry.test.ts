...
import { checkRequestedToolRegistration, getToolNameFromValue } from "#src/integrations/tool-registry";
...
describe("checkRequestedToolRegistration", () => {
  ...
  test("returns registered when tool name matches an object entry by toolName", () => {
    const result = checkRequestedToolRegistration("bash", [{ toolName: "bash" }]);
    expect(result.status).toBe("registered");
  });
  ...
  test("available tool names are sorted alphabetically", () => {
    const result = checkRequestedToolRegistration("ghost", ["write", "read", "edit"]);
    if (result.status === "unregistered") {
  ...
  test("skips tool list entries that yield no name", () => {
    const result = checkRequestedToolRegistration("read", [null, {}, { unrelated: "x" }, "read"]);
    expect(result.status).toBe("registered");
  });
});

...
import { PermissionServiceLifecycle, type ServiceLifecycle } from "#src/integrations/service-lifecycle";
...
vi.mock("#src/forwarding/subagents/subagent-context", () => ({
...
function makeLifecycle(overrides?: { subscriptions?: (() => void)[] }) {
  ...
  const lifecycle = new PermissionServiceLifecycle({ service, registry, events, subscriptions });
...
describe("activate", () => {
  ...
  it("emits ready after publishing the service", () => {
    const ctx = makeCtx();
    const order: string[] = [];
    mockPublishPermissionsService.mockImplementation(() => order.push("publish"));
...
describe("teardown", () => {
  ...
  it("unpublishes the service after running subscriptions", () => {
    ...
    mockUnpublishPermissionsService.mockImplementation(() => order.push("unpublish"));
  ...
  it("does not run teardown side effects more than once", () => {
    const unsub = vi.fn();
    const { lifecycle } = makeLifecycle({ subscriptions: [unsub] });
    lifecycle.teardown();
    lifecycle.teardown();
  ...
  it("does not mark teardown complete when a subscription throws", () => {
    const unsub = vi.fn(() => {
      throw new Error("unsubscribe failed");
    });
    const { lifecycle } = makeLifecycle({ subscriptions: [unsub] });
    expect(() => lifecycle.teardown()).toThrow("unsubscribe failed");
    expect(() => lifecycle.teardown()).toThrow("unsubscribe failed");
    expect(unsub).toHaveBeenCalledTimes(2);
    expect(mockUnpublishPermissionsService).not.toHaveBeenCalled();
  });
});

...
describe("describeToolGate", () => {
  ...
  it("returns descriptor with tool name as decision value for standard tools", () => {
    const desc = describeToolGate(makeTcc({ toolName: "write" }), makeCheckResult("ask"), makeFormatter());
  ...
  it("returns mcp surface with target in decision.value for MCP tools", () => {
    ...
    const desc = describeToolGate(makeTcc({ toolName: "mcp", input: { tool: "server:tool" } }), check, makeFormatter());
  ...
  it("populates denialContext with agent name when provided", () => {
    ...
    const desc = describeToolGate(makeTcc({ agentName: "my-agent" }), check, makeFormatter());

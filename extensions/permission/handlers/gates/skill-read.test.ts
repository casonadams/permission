...
describe("describeSkillReadGate", () => {
  it("returns null when tool is not read", () => {
    const result = describeSkillReadGate(makeTcc({ toolName: "write" }), () => [makeSkillEntry()]);
  ...
  it("returns null when input has no path", () => {
    const result = describeSkillReadGate(makeTcc({ input: {} }), () => [makeSkillEntry()]);
    expect(result).toBeNull();
  });
  ...
  it("returns GateDescriptor with preResolved.state matching skill entry state (allow)", () => {
    const result = describeSkillReadGate(makeTcc(), () => [makeSkillEntry({ state: "allow" })]);
    expect(result).not.toBeNull();
    const desc = result!;
  ...
  it("returns GateDescriptor with preResolved.state matching skill entry state (deny)", () => {
    const result = describeSkillReadGate(makeTcc(), () => [makeSkillEntry({ state: "deny" })]);
    expect(result).not.toBeNull();
    const desc = result!;
  ...
  it("decision surface is 'skill' and decision value is the skill name", () => {
    const result = describeSkillReadGate(makeTcc(), () => [makeSkillEntry({ name: "my-skill" })])!;
  ...
  it("denialContext contains the skill name and read path", () => {
    const result = describeSkillReadGate(makeTcc(), () => [makeSkillEntry({ name: "librarian" })])!;

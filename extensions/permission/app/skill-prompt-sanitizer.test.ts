...
} from "#src/app/skill-prompt-sanitizer";
import type { PermissionManager } from "#src/policy/permission-manager";
...
function skillBlock(name: string, location = `/skills/${name}/SKILL.md`): string {
  return [
    "  <skill>",
    `    <name>${name}</name>`,
...
describe("resolveSkillPromptEntries", () => {
  ...
  test("delegates permission check to permissionManager for each skill", () => {
    ...
    expect(manager.checkPermission).toHaveBeenCalledWith("skill", { name: "alpha" }, undefined);
    expect(manager.checkPermission).toHaveBeenCalledWith("skill", { name: "beta" }, undefined);
  });
  ...
  test("passes agentName to permissionManager", () => {
    ...
    expect(manager.checkPermission).toHaveBeenCalledWith("skill", { name: "librarian" }, "my-agent");
  });
...
describe("findSkillPathMatch", () => {
  ...
  test("matches path within skill base directory", () => {
    const match = findSkillPathMatch("/skills/librarian/extra/helper.md", entries);
    expect(match?.name).toBe("librarian");
  });
...
test("REGRESSION: resolveSkillPromptEntries sanitizes every available_skills block", () => {
  ...
  try {
    ...
    expect(result.entries.map((entry) => entry.name)).toEqual(["visible-skill"]);
  } finally {
    cleanup();
  }
});

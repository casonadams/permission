...
import { SkillInputGatePipeline } from "#src/gates/skill-input-gate-pipeline";
...
import { makeCtx, makeEvents, makeToolRegistry } from "#test/helpers/handler-fixtures";
...
type CheckPermissionArgs = Parameters<ReturnType<typeof makeRealSession>["permissionManager"]["checkPermission"]>;

function resolveDedupPermissionCheck(...args: CheckPermissionArgs): PermissionCheckResult {
  const [surface, input, _agentName, rules] = args;
  if (surface !== "external_directory") return allowTool(surface);
  return resolveExternalDirectoryCheck(surface, input, rules);
}

function resolveExternalDirectoryCheck(
  surface: string,
  input: unknown,
  rules: CheckPermissionArgs[3],
): PermissionCheckResult {
  const pathValue = readPathValue(input);
  const match = pathValue
    ? rules?.findLast((r) => r.surface === "external_directory" && wildcardMatch(r.pattern, pathValue))
    : undefined;
  if (match) return allowSession(surface, match.pattern);
  return askExternalDirectory(surface);
}

function readPathValue(input: unknown): string | null {
  const record = (input ?? {}) as Record<string, unknown>;
  return typeof record.path === "string" ? record.path : null;
}

function allowSession(toolName: string, matchedPattern: string): PermissionCheckResult {
  return { state: "allow", toolName, source: "session", origin: "session", matchedPattern };
}

function askExternalDirectory(toolName: string): PermissionCheckResult {
  return { state: "ask", toolName, source: "special", origin: "global" };
}

function allowTool(toolName: string): PermissionCheckResult {
  return { state: "allow", toolName, source: "tool", origin: "builtin" };
}
...
function makeDeduplicatingHandler(prompter?: GatePrompter): {
  ...
  vi.mocked(permissionManager.checkPermission).mockImplementation(resolveDedupPermissionCheck);
  ...
  const runner = new GateRunner({ resolver, recorder: sessionRules, defaultPrompter: resolvedPrompter, reporter });
  const handler = new PermissionGateHandler({
    session,
    toolRegistry: makeToolRegistry({
      getAll: vi.fn().mockReturnValue([{ name: "read" }, { name: "write" }, { name: "edit" }, { name: "bash" }]),
    }),
    pipeline: new ToolCallGatePipeline({ resolver, inputs: session }),
...
describe("session shutdown clears external-directory approvals", () => {
  it("re-prompts for the same path after session shutdown", async () => {
    ...
    const runner = new GateRunner({ resolver, recorder: sessionRules, defaultPrompter: prompter, reporter });
    const handler = new PermissionGateHandler({
      session,
      toolRegistry: makeToolRegistry({

...
import { extractSkillNameFromInput } from "#src/app/handlers/permission-gate-handler";
...
describe("extractSkillNameFromInput", () => {
  ...
  it("extracts skill name stopping at whitespace", () => {
    expect(extractSkillNameFromInput("/skill:librarian some extra")).toBe("librarian");
  });
...
describe("handleInput", () => {
  ...
  it("returns continue for non-skill input", async () => {
    ...
    const result = await handler.handleInput(makeInputEvent("just a message"), makeCtx());
  ...
  it("prompts and returns continue when skill ask is approved", async () => {
    const approvePrompt = vi.fn<GatePrompter["prompt"]>().mockResolvedValue({ approved: true, state: "approved" });

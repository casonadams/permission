...
import { createBeforeAgentStartPromptStateKey } from "#src/app/before-agent-start-cache";
...
test("Before-agent-start prompt cache invalidates on permission changes while runtime enforcement stays authoritative", () => {
  ...
  try {
    ...
    const updatedConfig = `${JSON.stringify({ permission: { "*": "allow", write: "allow" } }, null, 2)}\n`;

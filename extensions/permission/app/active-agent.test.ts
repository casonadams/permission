...
describe("ACTIVE_AGENT_TAG_REGEX", () => {
  test("matches double-quoted name attribute", () => {
    const match = '<active_agent name="my-agent">'.match(ACTIVE_AGENT_TAG_REGEX);
...
describe("getActiveAgentName", () => {
  ...
  test("returns agent name from active_agent entry", () => {
    const ctx = makeCtx([{ type: "custom", customType: "active_agent", data: { name: "bot" } }]);
  ...
  test("ignores entries with wrong customType", () => {
    const ctx = makeCtx([{ type: "custom", customType: "something_else", data: { name: "bot" } }]);
    expect(getActiveAgentName(ctx)).toBeNull();
  });

  test("ignores entries with wrong type", () => {
    const ctx = makeCtx([{ type: "tool_call", customType: "active_agent", data: { name: "bot" } }]);
    expect(getActiveAgentName(ctx)).toBeNull();
  });
});
...
describe("getActiveAgentNameFromSystemPrompt", () => {
  ...
  test("returns null when tag is absent", () => {
    expect(getActiveAgentNameFromSystemPrompt("You are a helpful assistant.")).toBeNull();
  });

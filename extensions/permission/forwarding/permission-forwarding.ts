...
import type { PermissionUiPromptSource } from "../integrations/permission-events";
import type { PermissionDecisionState } from "../prompting/permission-dialog";
...
export type ForwardedPermissionRequest = {
  ...
  message: string;
  // Optional because newer parents can read requests written by older children during upgrades.
  source?: PermissionUiPromptSource;
  surface?: string | null;
  value?: string | null;
};
...
export interface ResolvePermissionForwardingTargetOptions {
  hasUI: boolean;
  isSubagent: boolean;
...
export function resolvePermissionForwardingTargetSessionId(
  options: ResolvePermissionForwardingTargetOptions,
): string | null {
  if (options.hasUI) return normalizePermissionForwardingSessionId(options.currentSessionId);
  if (!options.isSubagent) return null;
  return resolveRegistryParentSession(options) ?? resolveEnvParentSession(options.env ?? process.env);
}

function resolveRegistryParentSession(options: ResolvePermissionForwardingTargetOptions): string | null {
  if (!options.registry || !options.sessionId) return null;
  const entry = options.registry.get(options.sessionId);
  return normalizePermissionForwardingSessionId(entry?.parentSessionId);
}

function resolveEnvParentSession(env: NodeJS.ProcessEnv): string | null {

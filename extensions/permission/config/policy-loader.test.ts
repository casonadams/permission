...
function makeLoader(
  ...
  writeFileSync(globalConfigPath, JSON.stringify(options.globalConfig ?? {}, null, 2));
...
describe("FilePolicyLoader.loadProjectConfig", () => {
  ...
  it("returns ScopeConfig from a project config file", () => {
    ...
    try {
      ...
      writeFileSync(projectConfigPath, JSON.stringify({ permission: { bash: "allow" } }));
      const loader = new FilePolicyLoader({
        globalConfigPath: "/nonexistent/config.json",
        agentsDir: "/nonexistent/agents",
...
describe("FilePolicyLoader.getConfigIssues", () => {
  ...
  it("does not return stale issues from a different agent", () => {
    const baseDir = makeTempDir();
    try {
      const agentsDir = join(baseDir, "agents");
      mkdirSync(agentsDir, { recursive: true });
      ...
      writeFileSync(join(agentsDir, "clean.md"), "---\npermission:\n  bash: allow\n---\n# Clean\n");
      ...
      loader.loadGlobalConfig();
      loader.loadAgentConfig("bad");
      expect(loader.getConfigIssues("bad")).toEqual([
        "Invalid permission config at 'permission.bash': expected allow, deny, or ask",
      ]);

      loader.loadAgentConfig("clean");
      expect(loader.getConfigIssues("clean")).toEqual([]);
      expect(loader.getConfigIssues()).toEqual([]);
    } finally {
      rmSync(baseDir, { recursive: true, force: true });
    }
...
describe("FilePolicyLoader mtime caching", () => {
  ...
  it("reloads when file mtime changes", () => {
    ...
    try {
      ...
      const agentsDir = join(baseDir, "agents");
      mkdirSync(agentsDir, { recursive: true });
      writeFileSync(globalConfigPath, JSON.stringify({ permission: { "*": "allow" } }));
...
describe("FilePolicyLoader.getConfiguredMcpServerNames", () => {
  ...
  it("reads server names from mcp.json on disk", () => {
    ...
    try {
      ...
      writeFileSync(mcpConfigPath, JSON.stringify({ mcpServers: { exa: {}, research: {} } }));
...
describe("FilePolicyLoader config issues", () => {
  ...
  it("clears fixed config issues after reload", () => {
    const baseDir = makeTempDir();
    try {
      const globalConfigPath = join(baseDir, "config.json");
      const agentsDir = join(baseDir, "agents");

...
import { type DecisionReporter, GateDecisionReporter } from "#src/decision-reporter";
import { PERMISSIONS_DECISION_CHANNEL, type PermissionDecisionEvent } from "#src/permission-events";
...
describe("GateDecisionReporter", () => {
  it("satisfies the DecisionReporter interface", () => {
    const reporter: DecisionReporter = new GateDecisionReporter(makeLogger(), makeEvents());
  ...
  describe("writeReviewLog", () => {
    ...
    it("delegates with an empty details object", () => {
      ...
      expect(logger.review).toHaveBeenCalledWith("permission_request.session_approved", {});
    });
  ...
  describe("emitDecision", () => {
    it("emits on the PERMISSIONS_DECISION_CHANNEL with the event", () => {
      ...
      expect(events.emit).toHaveBeenCalledWith(PERMISSIONS_DECISION_CHANNEL, event);
    });

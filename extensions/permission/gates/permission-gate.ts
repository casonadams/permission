...
export async function applyPermissionGate(params: PermissionGateParams): Promise<PermissionGateResult> {
  const { state, writeLog, logContext, messages } = params;
...
async function handleAskState(params: PermissionGateParams): Promise<PermissionGateResult> {
  const { canConfirm, promptForApproval, writeLog, logContext, messages } = params;
  if (!canConfirm) {
    writeLog("permission_request.blocked", { ...logContext, resolution: "confirmation_unavailable" });

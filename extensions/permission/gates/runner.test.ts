...
import type { GateBypass } from "#src/gates/descriptor";
...
describe("GateRunner — descriptor path", () => {
  ...
  it("returns allow, emits user_approved_for_session, and records session rule on approved_for_session", async () => {
    ...
    expect(deps.recordSessionApproval).toHaveBeenCalledWith(SessionApproval.single("read", "*"));
  });
  ...
  it("passes requestId from toolCallId to prompt", async () => {
    ...
    expect(deps.prompt).toHaveBeenCalledWith(expect.objectContaining({ requestId: "tc-42" }));
  });

...
describe("extractExternalPathsFromBashCommand", () => {
  ...
  describe("absolute paths", () => {
    test("detects absolute path outside CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /etc/hosts", cwd);
      expect(result).toContain("/etc/hosts");
    });

    test("detects multiple absolute paths outside CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("diff /etc/hosts /var/log/syslog", cwd);
      expect(result).toContain("/etc/hosts");
      expect(result).toContain("/var/log/syslog");
    });

    test("does not flag absolute path within CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /projects/my-app/src/index.ts", cwd);
  ...
  describe("home-relative paths", () => {
    test("detects ~/path outside CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("cat ~/documents/secret.txt", cwd);
    ...
    test("does not flag ~/path that resolves within CWD", async () => {
      ...
      const result = await extractExternalPathsFromBashCommand("cat ~/myproject/file.ts", "/mock/home/myproject");
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("dot-dot relative paths", () => {
    test("detects ../ path that resolves outside CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("cat ../../other-project/secrets.env", cwd);
      expect(result).toContain("/other-project/secrets.env");
    });

    test("does not flag ../ path that stays within CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("cat src/../lib/utils.ts", cwd);
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("commands within CWD only", () => {
    test("returns empty for relative paths within CWD", async () => {
      const result = await extractExternalPathsFromBashCommand("cat src/index.ts", cwd);
      expect(result).toHaveLength(0);
    });

    test("returns empty for bare command with no path arguments", async () => {
      const result = await extractExternalPathsFromBashCommand("git status", cwd);
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("flags are skipped", () => {
    test("does not treat flags as paths", async () => {
      const result = await extractExternalPathsFromBashCommand("ls -la --color=auto", cwd);
      expect(result).toHaveLength(0);
    });

    test("detects path after flags", async () => {
      const result = await extractExternalPathsFromBashCommand("ls -la /etc/passwd", cwd);
  ...
  describe("env assignments are skipped", () => {
    test("does not treat FOO=/bar as a path", async () => {
      const result = await extractExternalPathsFromBashCommand("FOO=/usr/local/bin command", cwd);
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("shell metacharacters split correctly", () => {
    test("detects path after pipe", async () => {
      const result = await extractExternalPathsFromBashCommand("echo hello | tee /tmp/output.txt", cwd);
      expect(result).toContain("/tmp/output.txt");
    });

    test("detects path after semicolon", async () => {
      const result = await extractExternalPathsFromBashCommand("echo done; cat /etc/hosts", cwd);
      expect(result).toContain("/etc/hosts");
    });

    test("detects path after &&", async () => {
      const result = await extractExternalPathsFromBashCommand("true && cat /etc/hosts", cwd);
      expect(result).toContain("/etc/hosts");
    });

    test("detects path in redirect target", async () => {
      const result = await extractExternalPathsFromBashCommand("echo hello > /tmp/out.txt", cwd);
  ...
  describe("URLs are skipped", () => {
    test("does not treat http:// URL as a path", async () => {
      const result = await extractExternalPathsFromBashCommand("curl http://example.com/path", cwd);
      expect(result).toHaveLength(0);
    });

    test("does not treat https:// URL as a path", async () => {
      const result = await extractExternalPathsFromBashCommand("curl https://example.com/etc/hosts", cwd);
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("@scope/package patterns are skipped", () => {
    test("does not treat @scope/package as a path", async () => {
      const result = await extractExternalPathsFromBashCommand("npm install @types/node", cwd);
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("quoted strings are ignored", () => {
    test("does not flag path inside double-quoted string", async () => {
      const result = await extractExternalPathsFromBashCommand('git commit -m "fix: update /etc/hosts handler"', cwd);
      expect(result).toHaveLength(0);
    });

    test("does not flag path inside single-quoted string", async () => {
      const result = await extractExternalPathsFromBashCommand("echo 'see /usr/local/docs for info'", cwd);
      expect(result).toHaveLength(0);
    });

    test("still flags unquoted path alongside quoted content", async () => {
      const result = await extractExternalPathsFromBashCommand('cat /etc/hosts && echo "done"', cwd);
      expect(result).toContain("/etc/hosts");
    });
    ...
    test("does not flag path when adjacent quoted segments form one word", async () => {
      ...
      const result = await extractExternalPathsFromBashCommand('echo "path is "/etc/hosts""', cwd);
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("safe system paths are filtered", () => {
    test("does not flag /dev/null in stderr redirect", async () => {
      const result = await extractExternalPathsFromBashCommand("command 2>/dev/null", cwd);
      expect(result).toHaveLength(0);
    });

    test("does not flag /dev/null as a redirect target", async () => {
      const result = await extractExternalPathsFromBashCommand("echo hello > /dev/null", cwd);
      expect(result).toHaveLength(0);
    });

    test("does not flag /dev/stdin", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /dev/stdin", cwd);
      expect(result).toHaveLength(0);
    });

    test("does not flag /dev/stdout", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /dev/stdout", cwd);
      expect(result).toHaveLength(0);
    });

    test("does not flag /dev/stderr", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /dev/stderr", cwd);
      expect(result).toHaveLength(0);
    });

    test("still flags a real external path alongside /dev/null", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /etc/hosts 2>/dev/null", cwd);
      expect(result).toContain("/etc/hosts");
      expect(result).not.toContain("/dev/null");
    });

    test("does not flag /dev/null/subdir (not a safe path)", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /dev/null/subdir", cwd);
  ...
  describe("bare-slash tokens are skipped", () => {
    ...
    test("does not flag // in echo with other args", async () => {
      const result = await extractExternalPathsFromBashCommand("echo // hello", cwd);
      expect(result).toHaveLength(0);
    });
    ...
    test("still flags real external path alongside //", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /etc/hosts; echo //", cwd);
      expect(result).toContain("/etc/hosts");
  ...
  describe("tokenizer edge cases", () => {
    ...
    test("does not flag path appearing only in a shell comment", async () => {
      const result = await extractExternalPathsFromBashCommand("echo hello # /etc/shadow", cwd);
      expect(result).toHaveLength(0);
    });

    test("flags real path before comment but not path inside comment", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /etc/hosts # see also /etc/shadow", cwd);
      expect(result).toContain("/etc/hosts");
  ...
  describe("command substitution", () => {
    test("detects path inside command substitution", async () => {
      const result = await extractExternalPathsFromBashCommand("echo $(cat /etc/hosts)", cwd);
      expect(result).toContain("/etc/hosts");
    });

    test("detects path inside nested command substitution", async () => {
      const result = await extractExternalPathsFromBashCommand("echo $(echo $(cat /etc/hosts))", cwd);
      expect(result).toContain("/etc/hosts");
    });
  ...
  describe("redirect targets", () => {
    ...
    test("detects path in input redirect", async () => {
      const result = await extractExternalPathsFromBashCommand("sort < /etc/hosts", cwd);
      expect(result).toContain("/etc/hosts");
    });

    test("detects path in stderr redirect", async () => {
      const result = await extractExternalPathsFromBashCommand("command 2>/tmp/errors.log", cwd);
  ...
  describe("deduplication", () => {
    test("returns deduplicated paths", async () => {
      const result = await extractExternalPathsFromBashCommand("cat /etc/hosts; grep foo /etc/hosts", cwd);
  ...
  describe("command-aware extraction", () => {
    describe("sed", () => {
      ...
      test("sed script is skipped but file argument is extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("sed 's/foo/bar/g' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
      });

      test("sed address pattern starting with / is skipped", async () => {
        const result = await extractExternalPathsFromBashCommand("sed '/pattern/d' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });

      test("sed with only in-CWD file returns empty", async () => {
        const result = await extractExternalPathsFromBashCommand("sed 's/foo/bar/' src/index.ts", cwd);
        expect(result).toHaveLength(0);
      });

      test("sed -e: script consumed by flag, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("sed -e 's/foo/bar/' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });

      test("sed -n: regular flag does not consume next arg", async () => {
        const result = await extractExternalPathsFromBashCommand("sed -n '/pattern/p' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });

      test("sed -f: script file is extracted as path", async () => {
        const result = await extractExternalPathsFromBashCommand("sed -f /etc/sed-script.sed input.txt", cwd);
        expect(result).toContain("/etc/sed-script.sed");
        expect(result).toHaveLength(1);
      });

      test("sed -i '': extension consumed, script skipped, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("sed -i '' 's/foo/bar/' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });
    });
    ...
    describe("grep", () => {
      test("grep: pattern skipped, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("grep '/etc/' /var/log/syslog", cwd);
        expect(result).toContain("/var/log/syslog");
        expect(result).toHaveLength(1);
      });

      test("grep -e: pattern consumed by flag, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("grep -e '/etc/' /var/log/syslog", cwd);
        expect(result).toContain("/var/log/syslog");
        expect(result).toHaveLength(1);
      });
    });
    ...
    describe("awk", () => {
      test("awk: program skipped, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("awk '{print}' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });

      test("awk -F: separator consumed, program skipped, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("awk -F: '{print $1}' /etc/passwd", cwd);
        expect(result).toContain("/etc/passwd");
        expect(result).toHaveLength(1);
      });
    });
    ...
    describe("rg", () => {
      test("rg: pattern skipped, path extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("rg '/usr/local' /etc/profile.d/", cwd);
        expect(result).toContain("/etc/profile.d");
        expect(result).toHaveLength(1);
      });

      test("rg -e: pattern consumed by flag, path extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("rg -e '/usr/local' /etc/profile.d/", cwd);
        expect(result).toContain("/etc/profile.d");
        expect(result).toHaveLength(1);
      });
    });
    ...
    describe("sd", () => {
      test("sd: both pattern positionals skipped, file extracted", async () => {
        const result = await extractExternalPathsFromBashCommand("sd '/usr/local/bin' '/opt/bin' /etc/profile", cwd);
        expect(result).toContain("/etc/profile");
        expect(result).toHaveLength(1);
      });

      test("sd with only in-CWD file returns empty", async () => {
        const result = await extractExternalPathsFromBashCommand("sd 'foo' 'bar' src/index.ts", cwd);
        expect(result).toHaveLength(0);
      });
    });
    ...
    describe("unknown commands", () => {
      test("unknown command: all args go through generic extraction", async () => {
        const result = await extractExternalPathsFromBashCommand("some-tool /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
      });
    });
    ...
    describe("edge cases", () => {
      test("full-path command invocation: /usr/bin/sed", async () => {
        const result = await extractExternalPathsFromBashCommand("/usr/bin/sed 's/foo/bar/' /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });
      ...
      test("redirect target still extracted for pattern-first command", async () => {
        const result = await extractExternalPathsFromBashCommand("sed 's/foo/bar/' input.txt > /tmp/output.txt", cwd);
        expect(result).toContain("/tmp/output.txt");
      });

      test("pipeline: sed piped to cat with external path", async () => {
        const result = await extractExternalPathsFromBashCommand("sed 's/foo/bar/' src/file.ts | cat /etc/hosts", cwd);
        expect(result).toContain("/etc/hosts");
        expect(result).toHaveLength(1);
      });

      test("command substitution inside pattern-first command", async () => {
        const result = await extractExternalPathsFromBashCommand("grep 'pattern' $(cat /etc/file-list)", cwd);
  ...
  describe("regex patterns are not mistaken for paths", () => {
    ...
    test("grep -v with //.*pattern without backslash-pipe is not flagged", async () => {
      const result = await extractExternalPathsFromBashCommand('grep -v "//.*foo" file.txt', cwd);
      expect(result).toHaveLength(0);
    });

    test("grep with backslash-pipe alternation is not flagged", async () => {
      const result = await extractExternalPathsFromBashCommand('grep "foo\\|bar\\|baz" src/file.ts', cwd);
      expect(result).toHaveLength(0);
    });

    test("grep -E with ^/ anchored regex is not flagged", async () => {
      const result = await extractExternalPathsFromBashCommand('grep -E "^/usr/bin" file.txt', cwd);
      expect(result).toHaveLength(0);
    });

    test("sed with regex containing slashes is not flagged", async () => {
      const result = await extractExternalPathsFromBashCommand('sed "s/foo.*/bar/g" file.txt', cwd);
      expect(result).toHaveLength(0);
    });

    test("real external paths are still detected alongside regex args", async () => {
      const result = await extractExternalPathsFromBashCommand('grep -v "//.*pattern" /etc/hosts', cwd);
      expect(result).toContain("/etc/hosts");
    });
  });
  ...
  describe("leading cd prefix", () => {
    ...
    test("cd to subdir: relative path that stays inside cwd is not flagged", async () => {
      const result = await extractExternalPathsFromBashCommand("cd /projects/my-app/src && cat ../README.md", cwd);
      expect(result).toHaveLength(0);
    });
    ...
    test("cd to external dir: subsequent paths resolve against the (external) effective directory", async () => {
      ...
      const result = await extractExternalPathsFromBashCommand("cd /tmp && cat ../etc/hosts", cwd);
...
describe("formatBashExternalDirectoryAskPrompt", () => {
  ...
  test("shows multiple external paths", () => {
    const result = formatBashExternalDirectoryAskPrompt({
      ...
      externalPaths: ["/etc/hosts", "/var/log/syslog"],
      cwd: "/projects/my-app",
    });
    expect(result).toContain("/etc/hosts");
    expect(result).toContain("/var/log/syslog");

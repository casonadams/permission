...
import { describeBashExternalDirectoryGate } from "#src/gates/bash-external-directory";
import { BashProgram } from "#src/gates/bash-program";
import type { GateBypass, GateDescriptor, GateResult } from "#src/gates/descriptor";
...
describe("describeBashExternalDirectoryGate", () => {
  it("returns null when tool is not bash", async () => {
    const result = await describeGate(makeTcc({ toolName: "read" }), makeResolver(makeCheckResult("ask")));
    expect(result).toBeNull();
  });

  it("returns null when no CWD", async () => {
    const result = await describeGate(makeTcc({ cwd: undefined }), makeResolver(makeCheckResult("ask")));
    expect(result).toBeNull();
  });

  it("returns null when command has no external paths", async () => {
    const result = await describeGate(makeTcc({ input: { command: "ls -la" } }), makeResolver(makeCheckResult("ask")));
    expect(result).toBeNull();
  });

  it("returns GateBypass when all external paths are session-covered", async () => {
    const resolver = makeResolver(makeCheckResult("allow", { source: "session" }));
  ...
  it("returns GateBypass when all external paths are config-level allowed", async () => {
    ...
    resolver.resolve.mockImplementation((_surface: string, input: unknown) => {
      if ((input as Record<string, unknown>).path) return makeCheckResult("allow", { source: "special" });
  ...
  it("uses worst-check state from uncovered paths for preCheck (config deny > catch-all ask)", async () => {
    ...
    const resolver = makeResolver();
    resolver.resolve.mockImplementation((_surface: string, input: unknown) => {
      if ((input as Record<string, unknown>).path) return makeCheckResult("deny", { source: "special" });
  ...
  it("descriptor surface is 'external_directory'", async () => {
    const result = await describeGate(makeTcc(), makeResolver(makeCheckResult("ask")));
    const desc = result as GateDescriptor;
  ...
  it("config-allowed path is excluded; remaining ask path produces a descriptor", async () => {
    ...
    const result = await describeGate(makeTcc({ input: { command: "diff /outside/a.ts /outside/b.ts" } }), resolver);
    expect(isGateDescriptor(result)).toBe(true);
    const desc = result as GateDescriptor;
    expect(desc.sessionApproval).toBeDefined();
    if (!desc.sessionApproval) return;
